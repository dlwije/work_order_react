    ...
    <div class="card" id="labortask_list_view">
        <div class="card-header">
            ...
            <div class="card-tools">
                @can('vehicle-type-add')
                    <a class="btn btn-sm btn-success no-shadow" href="javascript:0" onclick="vehicleTypeList.ivm.addNew()">  <i class="glyphicon glyphicon-plus myicon-right"></i> Add New</a>
                @endcan
            </div>
        </div>
        <!-- /.card-header -->
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-footer justify-content-between">
                        ...
                        <button type="button" class="btn btn-success" onclick="vehicleTypeList.ivm.submitTaskCategoryData()">Submit</button>
                    </div>
                </div>
                <!-- /.modal-content -->
            </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                vehicleTypeList.init();
                //start top progress bar
                // Pace.restart();

                $('#task_edit_mod').on('hide.bs.modal', function () {
                    vehicleTypeList.ivm.clearModelData();
                })
            })
            vehicleTypeList = {
                ivm: null,
                init: function () {
                    vehicleTypeList.ivm = new vehicleTypeList.vehicleTypeListViewModel();

                    //binding the knockout to the dom
                    ko.applyBindings(vehicleTypeList.ivm, $('#labortask_list_view')[0]);

                    //initialize the page design needs when page loads
                    vehicleTypeList.ivm.designInitialize();
                },
                vehicleTypeListViewModel: function () {
                    let self = this;
                    ...
                    self.listTable = function () {
                        ...
                        var table = $('#id_lbr_task_tbl').DataTable({
                            processing: true,
                            serverSide: true,
                            ajax: "{{ route('vehicle.type.table.list') }}",
                            columns: [
                                {data: 'id', name: 'id'},
                    ...
                    self.editDetails = function (empID) {
                        self.tsk_id(empID);
                        $.ajax({
                            url: "{{ route('vehicle.type.selected.data') }}",
                            dataType: 'json',
                            type: 'get',
                            ...
                            success: function( data, textStatus, jQxhr ){
                                ...
                                if(data.count > 0){
                                    ...
                                    self.mod_tsk_cate_name(data.data[0].type_name);
                                    $('#task_edit_mod').modal('show')
                    ...
                    self.submitTaskCategoryData = () =>{
                        ...
                        let url = "{{ route('vehicle.type.edit.submit') }}";
                        if(self.tsk_id() == "")
                            url = '{{ route('vehicle.type.create.submit') }}';
                    ...
                    self.deleteRecord = function(fbID){
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.ajax({
                                    url: "{{ route('vehicle.type.destroy.submit') }}",
                                    dataType: 'json',
                                    type: 'post',
                                    contentType: 'application/x-www-form-urlencoded',
                                    data: {
