...
@section('content')
    <div class="container-fluid" id="wo_edit_view">
        <div class="row">
            <div class="col-md-12">
            ...
            <div class="col-md-12">
                ...
                <div class="card card-primary card-outline">
                    ...
                    <div class="card-footer">
                        <div class="">
                            <div class="form-group">
                                <button type="button" class="btn btn-success btn-sm float-right" onclick="workOrder.ivm.formSubmit();"><i class="fas fa-save"></i> Submit</button>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        workOrder = {
            ...
            init: function (){
                ...
                ko.applyBindings(workOrder.ivm,$('#wo_edit_view')[0]);
            ...
            workOrderViewModel: function (){
                ...
                self.WONo = ko.observable("{{ $editData->serial_no }}");
                self.woCustomer = ko.observable("{{ $editData->customer_id }}");
                self.woVehicle = ko.observable("{{ $editData->vehicle_id }}");
                self.woDate = ko.observable("{{ $editData->wo_date }}");
                self.woAppointment = ko.observable("{{ $editData->appointment_id }}");

                self.insurance = ko.observable("@if(isset($editData['ins_data']->ins_company_name)) {{ $editData['ins_data']->ins_company_name }} @endif");
                self.contract = ko.observable("@if(isset($editData['cont_data']->con_company_name)) {{ $editData['cont_data']->con_company_name }} @endif");
                self.warranty = ko.observable("@if(isset($editData['war_data']->war_company_name)) {{ $editData['war_data']->war_company_name }} @endif");

                let insStateVal = "{{ $editData->is_insurance }}";
                let contStateVal = "{{ $editData->is_contract }}";
                let warStateVal = "{{ $editData->is_warranty }}";
                let insState = contState = warState = false;

                if(insStateVal == 1) insState =true;
                if(contStateVal == 1) contState =true;
                if(warStateVal == 1) warState =true;

                self.is_insurance = ko.observable(insState);
                self.is_contract = ko.observable(contState);
                self.is_warranty = ko.observable(warState);
                ...
                self.current_mileage = ko.observable(`{{ $editData->mileage }}`);
                self.woRemark = ko.observable(`{{ $editData->reported_defect }}`);
                ...
                self.mod_wo_ins_id = ko.observable("@if(isset($editData['ins_data']->id)) {{ $editData['ins_data']->id }} @endif");
                self.mod_ins_comp_name = ko.observable("@if(isset($editData['ins_data']->ins_company_name)) {{ $editData['ins_data']->ins_company_name }} @endif");
                self.mod_ins_policy_no = ko.observable("@if(isset($editData['ins_data']->policy_no)) {{ $editData['ins_data']->policy_no }} @endif");
                self.mod_ins_expire_date = ko.observable("@if(isset($editData['ins_data']->expire_date)) {{ $editData['ins_data']->expire_date }} @endif");
                self.mod_ins_phone_no = ko.observable("@if(isset($editData['ins_data']->phone_number)) {{ $editData['ins_data']->phone_number }} @endif");

                //service contract Model variables
                self.mod_wo_cont_id = ko.observable("@if(isset($editData['cont_data']->id)) {{ $editData['cont_data']->id }} @endif");
                self.mod_cont_comp_name = ko.observable("@if(isset($editData['cont_data']->con_company_name)) {{ $editData['cont_data']->con_company_name }} @endif");
                self.mod_cont_policy_no = ko.observable("@if(isset($editData['cont_data']->contract_no)) {{ $editData['cont_data']->contract_no }} @endif");
                self.mod_cont_expire_date = ko.observable("@if(isset($editData['cont_data']->expire_date)) {{ $editData['cont_data']->expire_date }} @endif");
                self.mod_cont_phone_no = ko.observable("@if(isset($editData['cont_data']->phone_number)) {{ $editData['cont_data']->phone_number }} @endif");
                self.mod_cont_mileage = ko.observable("@if(isset($editData['cont_data']->mileage)) {{ $editData['cont_data']->mileage }} @endif");

                //vehicle warranty Model variables
                self.mod_wo_warr_id = ko.observable("@if(isset($editData['war_data']->id)) {{ $editData['war_data']->id }} @endif");
                self.mod_warr_comp_name = ko.observable("@if(isset($editData['war_data']->war_company_name)) {{ $editData['war_data']->war_company_name }} @endif");
                self.mod_warr_warranty_no = ko.observable("@if(isset($editData['war_data']->warranty_no)) {{ $editData['war_data']->warranty_no }} @endif");
                self.mod_warr_dop = ko.observable("@if(isset($editData['war_data']->date_of_purchase)) {{ $editData['war_data']->date_of_purchase }} @endif");
                self.mod_warr_expire_date = ko.observable("@if(isset($editData['war_data']->expire_date)) {{ $editData['war_data']->expire_date }} @endif");
                self.mod_warr_phone_no = ko.observable("@if(isset($editData['war_data']->phone_number)) {{ $editData['war_data']->phone_number }} @endif");
                ...
                self.addNewCustomer = () => {
                    ...
                    $.ajax({
                        ...
                        error: function (jqXHR, textStatus, errorThrown) {
                            ...
                            if(jqXHR.status == 403){
                                sweetAlertMsg('Permission Error!','Users do not have the right permissions to proceed.','warning');
                            }else {
                ...
                self.formSubmit = function () {
                    ...
                    if(self.isNew()){ self.insertWorkorderData(); }else { self.updateWorkorderData(); }
                };
                ...
                self.updateWorkorderData = () => {

                    if(!self.formValidation()) return;

                    $.ajax({
                        url: '{{route('workorder.edit.submit')}}',
                        type: 'POST',
                        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
                        dataType: 'json',
                        data: {
                            ...
                            "mod_con_id": self.mod_wo_cont_id(),
                            ...
                            "mod_war_id": self.mod_wo_warr_id(),
                        ...
                        success: (responseData) => {
                            ...
                            sweetAlertMsg('Success',responseData.message,'success')
                            setTimeout(function () {
                                location.href = "{{ route('workorder.list') }}";
                            }, 200)
                        },
                        error: (jqXHR, textStatus, errorThrown) => {
                            StopLoading();
