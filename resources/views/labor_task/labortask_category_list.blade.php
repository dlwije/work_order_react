...
@section('title','Labor Tasks Category')
@push('css')
...
@section('content')
    <div class="card" id="labortask_list_view">
        <div class="card-header">
            <h3 class="card-title">Labor Task Category</h3>
                ...
                laborTaskListViewModel: function () {
                    ...
                    self.listTable = function () {
                        ...
                        var table = $('#id_lbr_task_tbl').DataTable({
                            processing: true,
                            serverSide: true,
                            ajax: "{{ route('task.category.table.list') }}",
                            columns: [
                                {data: 'id', name: 'id'},
                    ...
                    self.editDetails = function (empID) {
                        self.tsk_id(empID);
                        $.ajax({
                            url: "{{ route('task.category.selected.data') }}",
                            dataType: 'json',
                            type: 'get',
                            ...
                            success: function( data, textStatus, jQxhr ){
                                ...
                                if(data.count > 0){
                                    ...
                                    self.mod_tsk_cate_name(data.data[0].task_category_name);
                                    $('#task_edit_mod').modal('show')
                    ...
                    self.submitTaskCategoryData = () =>{
                        ...
                        let url = "{{ route('task.category.edit.submit') }}";
                        if(self.tsk_id() == "")
                            url = '{{ route('task.category.create.submit') }}';
                        ...
                        $.ajax({
                            ...
                            data: {
                                "_token": "{{ csrf_token() }}",
                                item_id: self.tsk_id(),
                                "tsk_cate_code": self.mod_tsk_cate_code(),
                                "tsk_cate_name": self.mod_tsk_cate_name()
                            },
                            success: function( data, textStatus, jQxhr ){
                    ...
                    self.deleteRecord = function(fbID){
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.ajax({
                                    url: "{{ route('task.category.destroy.submit') }}",
                                    dataType: 'json',
                                    type: 'post',
                                    contentType: 'application/x-www-form-urlencoded',
                                    data: {
