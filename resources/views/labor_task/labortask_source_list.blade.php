...
@section('title','Labor Tasks Source')
@push('css')
...
@section('content')
    <div class="card" id="labortask_list_view">
        <div class="card-header">
            <h3 class="card-title">Labor Task Source</h3>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">Task Source</h4>
                ...
                laborTaskListViewModel: function () {
                    ...
                    self.listTable = function () {
                        ...
                        var table = $('#id_lbr_task_tbl').DataTable({
                            processing: true,
                            serverSide: true,
                            ajax: "{{ route('task.source.table.list') }}",
                            columns: [
                                {data: 'id', name: 'id'},
                    ...
                    self.editDetails = function (empID) {
                        self.tsk_id(empID);
                        $.ajax({
                            url: "{{ route('task.source.selected.data') }}",
                            dataType: 'json',
                            type: 'get',
                            ...
                            success: function( data, textStatus, jQxhr ){
                                ...
                                if(data.count > 0){
                                    ...
                                    self.mod_tsk_cate_name(data.data[0].task_source_name);
                                    $('#task_edit_mod').modal('show')
                    ...
                    self.submitTaskCategoryData = () =>{
                        ...
                        let url = "{{ route('task.source.edit.submit') }}";
                        if(self.tsk_id() == "")
                            url = '{{ route('task.source.create.submit') }}';
                        ...
                        $.ajax({
                            ...
                            data: {
                                "_token": "{{ csrf_token() }}",
                                item_id: self.tsk_id(),
                                "tsk_src_code": self.mod_tsk_cate_code(),
                                "tsk_src_name": self.mod_tsk_cate_name()
                            },
                            success: function( data, textStatus, jQxhr ){
                    ...
                    self.deleteRecord = function(fbID){
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                $.ajax({
                                    url: "{{ route('task.source.destroy.submit') }}",
                                    dataType: 'json',
                                    type: 'post',
                                    contentType: 'application/x-www-form-urlencoded',
                                    data: {
